...
OidcClient.prototype = {
  ...
  loadMetadataAsync: function () {
    ...
    if (settings.cache) {
      var metadata = settings.cache.get(settings.authority + '-metadata');
      if (metadata) {
        utility.log("OidcClient.loadMetadataFromCache")
  ...
  validateTokenAsync: function(token, nonce, expectedAudience){
    var self = this;

    return self.loadX509SigningKeyAsync().then(function (cert) {
      utility.log("loaded x509 signing key");
      var jws = new r.jws.JWS();
      if (jws.verifyJWSByPemX509Cert(token, cert)) {
        utility.log("verified x509 cert");
        var token_contents = JSON.parse(jws.parsedJWS.payloadS);

        if (nonce !== token_contents.nonce) {
          return generateError("Invalid nonce");
        }

        return self.loadMetadataAsync().then(function (metadata) {
          utility.log("loaded metadata, validating issuer / client id");

          if (token_contents.iss !== metadata.issuer) {
            return generateError("Invalid issuer");
          }

          if (expectedAudience && token_contents.aud !== expectedAudience) {
          ...
          var diff = now - token_contents.iat;
          if (diff > (5 * 60)) {
            return generateError("Token issued too long ago");
          }

          if (token_contents.exp < now) {
  ...
  validateAccessTokenAsync: function(access_token){
    return this.validateTokenAsync(access_token);
  },
  ...
  validateIdTokenAsync: function (id_token, nonce, access_token) {
    ...
    var self = this;
    var settings = self._settings;
    return this.validateTokenAsync(id_token, nonce, settings.client_id).then(function(token_contents){
      if (access_token && settings.load_user_profile) {
        // if we have an access token, then call user info endpoint
        return self.loadUserProfile(access_token, token_contents).then(function (profile) {
          if (profile.statusCode) {
            // profile is an http response, not a profile response
            return utility.copy(token_contents);
          } else {
            return utility.copy(profile, token_contents);
          }
        });
      } else {
  ...
  validateAccessTokenFromIdTokenHashAsync: function (id_token_contents, access_token) {
    utility.log("OidcClient.validateAccessTokenFromIdTokenHashAsync");
  ...
  getTokenFromCodeAsync: function (code) {
    ...
    return self.loadTokenEndpoint().then(function (token_endpoint) {
      utility.log("OidcClient.getTokenFromCodeAsync then");
      var config = {
        method: 'POST',
        form: {
  ...
  processResponseAsync: function (result, requestState) {
    ...
    if (request_state.oidc && request_state.oauth) {
      ...
      localPromise = self.getTokenFromCodeAsync(result.code).then(function (codeResult) {
        utility.log("OidcClient.processResponseAsync.getTokenFromCodeAsync.then ");
        result = codeResult;
        if(result.id_token && result.access_token){
          return self.validateIdTokenAndAccessTokenAsync(result.id_token, request_state.nonce, result.access_token);
        }
